...
KeyValueStore *KeyValueStore::MakeKVS(IndexType type, nvmm::GlobalPtr location,
    ...
    KVSMetricsConfig kvs_metrics_config; // configuration through environment
                                         // variables

    switch (type) {
    case RADIX_TREE: {
        RadixTreeMetrics *metrics = new RadixTreeMetrics(kvs_metrics_config);
        return new KVSRadixTree(location, base, user, heap_size, heap_id,
                                metrics);
    }
    case RADIX_TREE_TINY: {
        std::cout << "RADIX_TREE_TINY not implemented" << std::endl;
        return nullptr;
    }
    case HASH_TABLE: {
        std::cout << "HASH_TABLE: not implemented" << std::endl;
        return nullptr;
    }
    case INVALID:
    default:
...
KeyValueStore *KeyValueStore::MakeKVS(std::string type_str,
