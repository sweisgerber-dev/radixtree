...
/* This function inserts an element in an atomic manner
 *  if key does not no exist.
 *
 * Return value
...
int KVSRadixTree::FindOrCreate(char const *key, size_t const key_len,
    ...
    if (old_value.IsValid()) {
        ...
        // std::cout <<"KVSRadixTree::FindOrCreate(): Returning the found Entry"
        // << std::endl;
        ValBuf *val_p = (ValBuf *)mmgr_->GlobalToLocal(old_value.gptr());
        fam_invalidate(&val_p->size, sizeof(size_t));
        size_t val_size = val_p->size;
        if (ret_len < val_size) {
            std::cout << "  val buffer is too small: " << ret_len << " -> "
                      << val_size << std::endl;
            ret_len = val_size;
            return -1;
        }
        ...
        // std::cout << "FindOrCreate: successfully inserted "
        //     << std::string(key, key_len) << " = " << std::string(val,
        // val_len) << std::endl;
        return 0;
    }
