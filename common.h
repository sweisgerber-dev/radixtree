...
struct TagGptr {
    ...
    Mark mark() const { return gptr_ & 1; }

    Gptr gptr() const { return gptr_mask(gptr_); }
    ...
    Gptr gptr_mask(Gptr p) const { return p & ~(Gptr)1; }

    // valid == gptr is not null
    bool IsValid() const { return ((Gptr)gptr_).IsValid(); }
...
inline bool operator==(const TagGptr &pt1, const TagGptr &pt2) {
    return pt1.gptr() == pt2.gptr() && pt1.mark() == pt2.mark() &&
           pt1.tag() == pt2.tag();
}
