...
class RadixTree {
  ...
  public:
    struct Iter {
        ...
        uint64_t next_pos; // the next value or child ptr to visit; 0: value;
                           // >0: pos-1 is the index of the key in the child
                           // array
    ...
    // a radix tree is uniquely identified by the memory manager instance, the
    // heap id, and the root pointer
    // when Root=0, create a new radix tree with the provied memory manager and
    // heap; get_root() will return the root pointer
    // when Root!=0, open an existing radix tree whose root pointer is Root,
    // with the provied memory manager and heap
    ...
    int scan(Iter &iter, char *key, size_t &key_size, TagGptr &value,
    ...
    int get_next(Iter &iter, char *key, size_t &key_size, TagGptr &value);
    ...
    /*
      ...
      - when we say "key was deleted", we mean the key NODE still exists but the
      value pointer was
      set to null with a valid version number
      - old_value is the previous value pointer in the key node before put or
      destroy, or null with
    ...
    */
    ...
    std::pair<Gptr, TagGptr> putC(const char *key, const size_t key_size,
    ...
    // the value ptr will be null with a valid version if the key was deleted or
    ...
    // or the key did not exist
    // the key ptr will be null if the key did not exist
    std::pair<Gptr, TagGptr> destroyC(const char *key, const size_t key_size,
                                      TagGptr &old_value);
  ...
  private:
    // when under high contention, current heap implementation may return 0 even
    // if there is free
    ...
    void recursive_list(Gptr parent,
                        std::function<void(const char *, const size_t, Gptr)> f,
                        uint64_t &level, uint64_t &depth, uint64_t &value_cnt,
                        uint64_t &node_cnt);
