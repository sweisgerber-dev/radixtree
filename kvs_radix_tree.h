...
class KVSRadixTree : public KeyValueStore {
  public:
    ...
    KVSRadixTree(Gptr root, std::string base, std::string user,
                 size_t heap_size, nvmm::PoolId heap_id,
                 RadixTreeMetrics *kvs_metrics);
    ~KVSRadixTree();

    void Maintenance();

    int Put(char const *key, size_t const key_len, char const *val,
            size_t const val_len);

    int Get(char const *key, size_t const key_len, char *val, size_t &val_len);

    int FindOrCreate(char const *key, size_t const key_len, char const *val,
                     size_t const val_len, char *ret_val, size_t &ret_len);

    int Del(char const *key, size_t const key_len);

    int Scan(int &iter_handle, char *key, size_t &key_len, char *val,
             size_t &val_len, char const *begin_key, size_t const begin_key_len,
             bool const begin_key_inclusive, char const *end_key,
             size_t const end_key_len, bool const end_key_inclusive);

    int GetNext(int iter_handle, char *key, size_t &key_len, char *val,
                size_t &val_len);
    ...
    */
    ...
    // for cached put
    int Put(Gptr const key_ptr, TagGptr &val_ptr, char const *val,
            size_t const val_len);
    ...
    // for cached Get
    // set get_value to true if one wants to fetch the value regardlessly (for
    // shortcut caching)
    // otherwise, only when the given val_ptr is stale we fetch the value from
    // FAM (for full caching)
    // return 1 if the val_ptr is up-to-date
    int Get(Gptr const key_ptr, TagGptr &val_ptr, char *val, size_t &val_len,
            bool get_value = false);

    // for non-cached Del
    int Del(char const *key, size_t const key_len, Gptr &key_ptr,
            TagGptr &val_ptr);
